...
if __name__ == '__main__':
    ...
    for x in range(3,40):#修改表格月份和天数
        if "月份" in str(path_sheet.cell(row=path_date_rownum-2, column=x).value):
            path_sheet.cell(row=path_date_rownum-2, column=x+1,value=str_year+"年"+str_month+"月")
        if x <= monthRange[1] + 2:
    ...
    for x in range(4,40):
        if "月份" in str(health_sheet.cell(row=health_date_rownum-2, column=x).value):
            health_sheet.cell(row=health_date_rownum-2, column=x+2,value=str_year+"年"+str_month+"月")
        if x <= monthRange[1] + 3:
            health_sheet.cell(row=health_date_rownum, column=x, value=str(x-3)+"日")
                ...
                if valid_sec ==0:
                    if count_true >2 : 
                        for k in range(count_true):
                            health_sheet.cell(row= i - path_date_rownum + health_date_rownum , column = begin_true+k-path_date_colnum+health_date_colnum,value="D")
                    elif count_true ==2:
                        for k in range(count_true):
                            health_sheet.cell(row= i - path_date_rownum + health_date_rownum , column = begin_true+k-path_date_colnum+health_date_colnum,value="B")
                    else:
                        health_sheet.cell(row= i - path_date_rownum + health_date_rownum , column = begin_true-path_date_colnum+health_date_colnum,value=random.choice(["C","E"]))
